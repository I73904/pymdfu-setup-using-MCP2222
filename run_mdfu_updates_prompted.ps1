...
function Prompt-Path([string]$label, [string]$defaultValue) {
    $value = Read-Host "$label [$defaultValue]"
    if ([string]::IsNullOrWhiteSpace($value)) { return $defaultValue }
    return $value
}

function Prompt-List([string]$label, [int[]]$defaultList) {
    $defaultText = ($defaultList -join ",")
    ...
    return $parts | ForEach-Object { [int]($_ -replace "_","") }
}

function Prompt-Protocols([string]$label, [string[]]$defaultList) {
    $defaultText = ($defaultList -join ",")
    $value = Read-Host "$label [$defaultText]"
    if ([string]::IsNullOrWhiteSpace($value)) { return $defaultList }
    return $value -split "," | ForEach-Object { $_.Trim().ToUpper() } | Where-Object { $_ -in @("UART","SPI","I2C") }
}
...
$PymdfuExe = Prompt-Path "Path to pymdfu.exe" $PymdfuExe
$ComPort = Prompt-Path "UART COM port (CDC)" $DefaultComPort

$protocolsToRun = Prompt-Protocols "Protocols to run (UART,SPI,I2C)" @("UART","SPI","I2C")
$UartBauds = Prompt-List "UART baud rates (comma-separated)" $UartBauds
$SpiSpeeds = Prompt-List "SPI speeds (comma-separated, Hz)" $SpiSpeeds
$I2cSpeeds = Prompt-List "I2C speeds (comma-separated, Hz)" $I2cSpeeds

$UartImage = Prompt-Path "UART image path" "$PSScriptRoot\uart\crc32\avr128da48-application-crc32.X\new_application.img"
$SpiImage  = Prompt-Path "SPI image path"  "$PSScriptRoot\spi\avr128da48-application-crc32.X\new_application.img"
$I2cImage  = Prompt-Path "I2C image path"  "$PSScriptRoot\i2c\avr128da48-application-crc32.X\new_application.img"

$UartBootloaderHex = Prompt-Path "UART bootloader HEX path" "$PSScriptRoot\uart\crc32\avr128da48-mdfu-client-crc32.X\dist\free\production\avr128da48-mdfu-client-crc32.X.production.hex"
$SpiBootloaderHex  = Prompt-Path "SPI bootloader HEX path"  "$PSScriptRoot\spi\avr128da48-mdfu-client-crc32.X\dist\free\production\avr128da48-mdfu-client-crc32.X.production.hex"
$I2cBootloaderHex  = Prompt-Path "I2C bootloader HEX path"  "$PSScriptRoot\i2c\avr128da48-mdfu-client-crc32.X\dist\free\production\avr128da48-mdfu-client-crc32.X.production.hex"

Assert-File $PymdfuExe "pymdfu.exe"
if ($protocolsToRun -contains "UART") { Assert-File $UartImage "UART image" }
if ($protocolsToRun -contains "SPI")  { Assert-File $SpiImage "SPI image" }
if ($protocolsToRun -contains "I2C")  { Assert-File $I2cImage "I2C image" }
...
$results = @()
if ($protocolsToRun -contains "UART") {
...
if ($protocolsToRun -contains "SPI") {
...
if ($protocolsToRun -contains "I2C") {
